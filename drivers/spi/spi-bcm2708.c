...
irqreturn_t bcm2708_transfer_one_message_dma_interrupts(int irq, void* dev) {
...
static int bcm2708_transfer_one_message_irqdriven_interrupts(int irq, struct spi_master *master) {
	char b;
	struct bcm2708_spi *bs = spi_master_get_devdata(master);
	spin_lock(&bs->lock);
...
{ 
	...
	struct bcm2708_spi *bs = spi_master_get_devdata(master);
	/* read the CS flags and based on those call the "correct" handler */
	u32 cs=bcm2708_rd(bs, SPI_CS);
	if (cs&SPI_CS_DMAEN) { return bcm2708_transfer_one_message_dma_interrupts(irq,master); }
	if (cs&SPI_CS_INTR) { return bcm2708_transfer_one_message_irqdriven_interrupts(irq,master); }
	/* return IRQ handled */
	return IRQ_HANDLED;
}
...
static int __devinit bcm2708_spi_probe(struct platform_device *pdev)
{
	...
	err = request_irq(bs->dma_rx.irq,
			bcm2708_transfer_one_message_dma_interrupts, 
			0, 
			dev_name(&pdev->dev),
			master);
	if (err) {
